...
class _PersonalChatScreenState extends State<PersonalChatScreen> {
        ...
        children: [
          ...
          Expanded(
            child: GridView.builder(
              ...
              itemCount: emojiData[emojiCategories[selectedEmojiCategoryIndex]]
                      ?.length ??
                  0,
              itemBuilder: (context, index) {
                final emoji = emojiData[
                    emojiCategories[selectedEmojiCategoryIndex]]![index];
                return GestureDetector(
  ...
  Widget build(BuildContext context) {
    ...
    final displayAvatar =
        widget.chat.getDisplayAvatar(controller.currentUserId);
            ...
            child: Obx(() {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  if (widget.chat.participantDetails
                      .containsKey(message.senderId)) {
                    final participantInfo =
                        widget.chat.participantDetails[message.senderId]!;
  ...
  Widget _buildEnhancedChatAvatar() {
    ...
    if (widget.chat.isGroup) {
      ...
      print(
          '   Group Image: ${groupImage != null ? "Found (${groupImage.length} chars)" : "None"}');
  ...
  Widget _buildGroupImageWidget(String? groupImage) {
    ...
    print(
        '   Image: ${groupImage != null ? "${groupImage.length} chars" : "null"}');
  ...
  String _getImageUrlFromMessage(ChatMessage message) {
    try {
      ...
      if (url != null && url.isNotEmpty) {
        print(
            '✅ Found image URL via getImageUrl(): ${url.length > 50 ? "${url.substring(0, 50)}..." : url}');
        return url;
      }
      ...
      if (message.imageUrl != null && message.imageUrl!.isNotEmpty) {
        print(
            '✅ Found imageUrl property: ${message.imageUrl!.length > 50 ? "${message.imageUrl!.substring(0, 50)}..." : message.imageUrl}');
...
// REPLACE your existing _buildMessageContent method in PersonalChatScreen with this:
  ...
  Widget _buildMessageStatusIcon(ChatMessage message) {
    try {
      ...
      final anyOnline =
          otherParticipants.any((id) => controller.isUserActuallyOnline(id));
